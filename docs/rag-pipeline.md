...
Generic overview requests such as “What is this document about?” use the first representative passages from the selected documents. These questions intentionally contain few semantic clues, so forcing them through the normal similarity threshold would incorrectly reject useful evidence. Factual questions continue to use cosine retrieval and the configured threshold.
